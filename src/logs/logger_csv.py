        ...
        if not Path(self.caminho_log).exists():
            with open(self.caminho_log, "w", newline="", encoding="utf-8") as f:
                csv.writer(f).writerow(
                    ["timestamp", "metodo", "entidade", "status", "registros_retornados", "caminho_destino", "erro"]
                )

    def registrar_sucesso(self, entidade: str, caminho: str, linhas: int) -> None:
        self._escrever(entidade, "sucesso", caminho, linhas, "")

    def registrar_erro(self, entidade: str, erro: str) -> None:
        self._escrever(entidade, "erro", "", 0, erro)

    def _escrever(self, entidade: str, status: str, caminho: str, linhas: int, erro: str) -> None:
